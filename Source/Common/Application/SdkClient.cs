...
namespace Common
{
    public class SdkClient
    {
        ...
        {
            var many = await _api.HttpGet<int>(endpoint, criteria);
            return many;
        }
        ...
        public async Task<bool> Exists(string endpoint, Guid id)
            => await Get<bool>(endpoint, id.ToString());

        public async Task<bool> Exists(string endpoint, Guid id1, Guid id2)
        ...
        {
            var many = await _api.HttpGet<IEnumerable<T>>(endpoint, criteria);
            Pagination = _api.Pagination;
            return many;
        }

        public async Task<IEnumerable<T>> Export<T>(string endpoint, Dictionary<string, string> criteria)
        {
            var many = await _api.HttpPost<IEnumerable<T>>(endpoint, criteria);
            Pagination = _api.Pagination;
            return many;
        }
        ...
        public Dictionary<string, string> ToDictionary(object obj)
            => DictionaryConverter.ToDictionary(obj);
    }
}