...
namespace Common.Observation
{
    ...
    public class EventPublisher : IEventPublisher
    {
        ...
        /// </summary>
        private readonly Dictionary<(string, Guid), Action<IEvent>> _precursors, _extenders;
        ...
        public EventPublisher()
        {
            ...
            _precursors = new Dictionary<(string, Guid), Action<IEvent>>();
            _extenders = new Dictionary<(string, Guid), Action<IEvent>>();
        ...
        /// </summary>
        public void Extend<T>(Action<T> action, Guid organization, bool before = false) where T : IEvent
        {
            var name = _reflector.GetClassName(typeof(T));
        ...
        public bool Extends<T>(Guid organization)
            => _extenders.Any(x => x.Key.Item1 == _reflector.GetClassName(typeof(T)) && x.Key.Item2 == organization);

        /// <summary>
        /// Register a custom organization-specific handler for the event.
        /// </summary>
        public void Override<T>(Action<T> action, Guid organization) where T : IEvent
        {
            var name = _reflector.GetClassName(typeof(T));
