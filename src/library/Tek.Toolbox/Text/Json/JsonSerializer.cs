...
namespace Tek.Toolbox
{
    ...
    {
        static readonly Dictionary<string, JsonResolver> Resolvers = new Dictionary<string, JsonResolver>();
        ...
        private JsonResolver GetResolver(bool disablePropertyConverters, string[] excludeProperties)
        {
            ...
            lock (this)
            {
                if (!Resolvers.ContainsKey(resolverKey))
                    Resolvers.Add(resolverKey, new JsonResolver(disablePropertyConverters, excludeProperties));
            }
