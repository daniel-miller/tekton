...
public class TProvinceReader : IEntityReader
{
    ...
    private IQueryable<TProvinceEntity> BuildQuery(TableDbContext db, IProvinceCriteria criteria)
    {
        ...
        // if (criteria.ProvinceNameTranslation != null)
        //    query = query.Where(x => x.ProvinceNameTranslation == criteria.ProvinceNameTranslation);

        if (criteria.CountryCode != null)
           query = query.Where(x => EF.Functions.ILike(x.CountryCode, criteria.CountryCode));
