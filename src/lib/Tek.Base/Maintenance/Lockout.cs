    ...
    public class Lockout : Model
    {
        ...
        public IEnumerable<ValidationError> Validate()
        {
            ...
            foreach (var i in Environments)
                if (i.MatchesNone(Base.Environments.Names))
                    errors.Add(new ValidationError { Property = nameof(Environments), Summary = $"Environments can contain only items in this list: {string.Join("; ", Base.Environments.Names)}" });
